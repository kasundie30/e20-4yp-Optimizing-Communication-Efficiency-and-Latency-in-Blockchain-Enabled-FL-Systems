...
#     (C) Error Feedback
#     (D) Int8 quantization on sparse values
# + LATENCY OPTIMIZATIONS:
#     (T1) Straggler-aware client selection (fast prioritized + exploration)
#     (T2) Deadline-based aggregation (cap processed updates)
#     (T3) Two-stage committee verification (cheap pre-check + expensive check for top candidates)
#     (T4) Adaptive verification set size schedule (bigger early, smaller later)
#
# Goal:
#   Reduce communication + latency WITHOUT sacrificing PR-AUC/F1/ACC/AUC
#
# Output log format:
...
RUN_NAME = os.environ.get("WANDB_RUN_NAME", f"litechain_full_comm_latency_{int(time.time())}")
...
INTERCLUSTER_EVERY = int(os.environ.get("INTERCLUSTER_EVERY", "2"))
...
PARTICIPATION_RATE = float(os.environ.get("PARTICIPATION_RATE", "0.6"))
...
# (T2) Deadline-based aggregation (cap processed updates per cluster per round)
MAX_UPDATES_PER_CLUSTER = int(os.environ.get("MAX_UPDATES_PER_CLUSTER", "6"))

# (T3) Two-stage verification
STAGE1_SIZE = int(os.environ.get("STAGE1_SIZE", "2500"))
STAGE2_MAX  = int(os.environ.get("STAGE2_MAX", "6"))
STAGE1_PR_AUC_MIN = float(os.environ.get("STAGE1_PR_AUC_MIN", "0.02"))

# (T4) Adaptive stage2 verification size schedule
STAGE2_SIZE_MAX = int(os.environ.get("STAGE2_SIZE_MAX", "15000"))
STAGE2_SIZE_MIN = int(os.environ.get("STAGE2_SIZE_MIN", "6000"))
STAGE2_DECAY    = float(os.environ.get("STAGE2_DECAY", "0.12"))

# --------- Communication techniques (Top-K + EF + Int8) ----------
USE_TOPK_EFQ = os.environ.get("USE_TOPK_EFQ", "1") == "1"
USE_INT8_SPARSE = os.environ.get("USE_INT8_SPARSE", "1") == "1"

TOPK_FRAC_MAX = float(os.environ.get("TOPK_FRAC_MAX", "0.05"))  # less aggressive early
TOPK_FRAC_MIN = float(os.environ.get("TOPK_FRAC_MIN", "0.01"))  # more aggressive later
TOPK_DECAY    = float(os.environ.get("TOPK_DECAY", "0.15"))
...
RESIDUAL_FP16 = os.environ.get("RESIDUAL_FP16", "1") == "1"

# Per-round threshold tuning
...
# ============================================================
# 3B) Communication compression: Top-K + Error Feedback + Int8
# ============================================================
...
def compress_delta_topk_efq(
    ...
    for k, t in delta.items():
        u = t.detach()
        if k in residual:
            # residual may be fp16; this is OK
            u = u + residual[k].to(u.dtype)
        ...
        # reconstruct sparse tensor (sender side) to update residual
...
def topk_frac_for_round(round_id: int) -> float:
    """
    Stability safeguard:
      - early rounds: higher TopK fraction (less aggressive)
      - later rounds: lower fraction (more compression)
    """
    if round_id <= WARMUP_ROUNDS:
        return TOPK_FRAC_MAX
    r = float(round_id - WARMUP_ROUNDS)
    frac = TOPK_FRAC_MIN + (TOPK_FRAC_MAX - TOPK_FRAC_MIN) * math.exp(-TOPK_DECAY * r)
    return float(max(TOPK_FRAC_MIN, min(TOPK_FRAC_MAX, frac)))
...
class LiteChainLedger:
    ...
    Important stability fix:
      Duplicate prevention is per-cluster: (cluster_id, model_hash).
      Otherwise identical models across different clusters could be rejected incorrectly.
    """
    def __init__(self):
        self.blocks: List[Block] = []
        self.seen_model_hashes = set()  # set of (cluster_id, model_hash)
    ...
    def append_block(self, blk: Block):
        ...
        self.seen_model_hashes.add((blk.cluster_id, blk.model_hash))
...
def cbft_consensus_algorithm4(
    ...
    # Duplicate protection (per cluster)
    if (payload["cluster_id"], payload["model_hash"]) in ledger.seen_model_hashes:
...
def neighbor_clusters_for_device(i: int, clusters: Dict[int, List[int]], k_from: int,
                                 est_bytes_per_update: int, committee_size: int) -> List[int]:
    scores = []
...
def algorithm2_update_preference_list(
    ...
    prefs = []
    for k_to in neigh:
        g = switch_gain_alg1(i, k_from, k_to, clusters, est_bytes_per_update, committee_size,
                             last_quality_by_cluster, reputations)
        if g > 0.0:
            prefs.append((g, k_to))
    ...
    for _ in range(max_slots):
        ...
        if not prefs:
            ...
            for ii in range(num_clients):
                ...
                for kt in neigh:
                    g = switch_gain_alg1(ii, kf, kt, clusters, est_bytes_per_update, committee_size,
                                         last_quality_by_cluster, reputations)
                    if g > 0.0:
                        stable = False
                        break
        ...
        g, k_to = prefs[0]
        if g <= prev_gain[i] or cluster_state.get(k_to, "available") != "available":
            continue
...
def stage2_size_for_round(round_idx_1based: int) -> int:
    ...
    # warmup: keep stage2 large early
    if round_idx_1based <= WARMUP_ROUNDS:
        size = STAGE2_SIZE_MAX
...
def select_clients_straggler_aware(
    ...
    """
    (T1) Straggler-aware:
      - pick FAST_SELECT_FRAC from fastest pool (lowest EMA latency)
      - rest random from remaining for exploration/fairness
    """
    if k_select >= len(client_ids):
        return list(client_ids)

    rng = np.random.default_rng(round_seed)
    scored = [(float(client_latency_ema.get(cid, 1.0)), cid) for cid in client_ids]
...
def deadline_cap_by_latency(selected_clients: List[int], client_latency_ema: Dict[int, float]) -> List[int]:
    """
    (T2) Deadline-based aggregation approximation:
      keep only fastest MAX_UPDATES_PER_CLUSTER clients (expected to "arrive" before deadline).
    """
    if MAX_UPDATES_PER_CLUSTER <= 0:
        return []
    if len(selected_clients) <= MAX_UPDATES_PER_CLUSTER:
        return selected_clients
    sel = sorted(selected_clients, key=lambda cid: float(client_latency_ema.get(cid, 1e9)))
    return sel[:MAX_UPDATES_PER_CLUSTER]


# ============================================================
# Verification subset helper (STRATIFIED) – stability critical
# ============================================================

def make_stratified_subset(X, y, size, seed, min_pos=30):
    """
    Ensures verification subset contains enough positive (fraud) samples,
    preventing PR-AUC from becoming None due to missing class.
    """
    y = np.asarray(y)
    ...
    n_pos = min(len(pos), max(1, min(min_pos, size // 2)))
...
config_dict = {
    ...
    "use_int8_sparse": USE_INT8_SPARSE,
    "topk_frac_max": TOPK_FRAC_MAX,
    "topk_frac_min": TOPK_FRAC_MIN,
...
# ---- Estimate bytes per update for Alg1 latency proxy (keep stable baseline estimate) ----
tmpm = new_model()
param_count = sum(p.numel() for p in tmpm.parameters())
EST_BYTES_PER_UPDATE = int(param_count * 4)  # dense float32 baseline estimate
...
for r in range(NUM_ROUNDS):
    ...
    topk_frac = topk_frac_for_round(round_id)

    # ============================================================
    # Algorithm 3: Intra-cluster training (partial participation + comm + latency)
    # ============================================================
    for k, cluster_client_ids in clusters.items():
        ...
        # (T2) deadline cap using latency ordering
        selected_clients = deadline_cap_by_latency(selected_clients, client_latency_ema)

        if len(selected_clients) == 0:
            continue

        # --- Two-stage verification subsets (STRATIFIED) ---
        Xc_s1, yc_s1 = make_stratified_subset(X_val, y_val, STAGE1_SIZE, seed=round_id*1000 + k, min_pos=25)
        Xc_s2, yc_s2 = make_stratified_subset(X_val, y_val, stage2_size, seed=round_id*2000 + k, min_pos=60)
        ...
        for idx, cid in enumerate(selected_clients):
            ...
            if USE_TOPK_EFQ:
                packets, new_res, est_bytes = compress_delta_topk_efq(
                    ...
                    topk_min_k=TOPK_MIN_K,
                    topk_max_k=TOPK_MAX_K,
                    use_int8_values=USE_INT8_SPARSE
                )
                # store residual
                if RESIDUAL_FP16:
                    residuals[cid] = {kk: vv.to(torch.float16) for kk, vv in new_res.items()}
                else:
                    residuals[cid] = new_res

                comm_client_to_committee += est_bytes
                d_recv = decompress_delta_topk_efq(packets, device=DEVICE, use_int8_values=USE_INT8_SPARSE)
            else:
                comm_client_to_committee += sum(int(t.numel()) * 4 for t in d.values())
                d_recv = d
            ...
            stage1_dt = time.time() - t1
            t_cluster_verify += stage1_dt

            norm = update_l2_norm(d_recv)

            # Update reputation raw stats
            ...
            # Update latency EMA (local train + stage1 verify)
            observed_lat = float(dt) + float(stage1_dt)
        ...
        # --- Stage 1 pre-filter: signature + loose PR-AUC + acc + norm ---
        stage1_pass = []
        for i_idx, cid in enumerate(selected_clients):
            ...
            pr1 = float(client_stage1[i_idx].get("pr_auc") or -1.0)
        ...
        # --- Stage 2: expensive verify only for top candidates (T3) ---
        ...
        for i_idx in stage2_idx:
            ...
            t_cluster_verify += (time.time() - t2)
            client_stage2[i_idx] = met2

        # ============================================================
        # Final accept rule (STAGE-2 GATED) – key for metric stability
        #   - normally accept ONLY those stage2-verified candidates that pass strict thresholds
        #   - fallback: if none accepted, accept best signature-valid stage1 candidate
        # ============================================================
        accepted_idx = []
        accepted_client_ids = []

        # strict acceptance over stage2 only
        for i_idx in stage2_idx:
            cid = selected_clients[i_idx]
            met = client_stage2[i_idx]
            if met is None:
                continue
        ...
        if len(accepted_idx) == 0:
            ...
            for i_idx, cid in enumerate(selected_clients):
                cand_hash, sig = client_sig[i_idx]
                if not verify_signature(client_secrets[cid], cand_hash, sig):
                    continue
                pr_val = float(client_stage1[i_idx].get("pr_auc") or -1.0)
                # keep norm cutoff on fallback too
                if client_norms[i_idx] <= cutoff and pr_val > best_pr:
                    best_pr = pr_val
                    best_i = i_idx
            if best_i is None:
                best_i = 0
        ...
        # ----- Aggregate verified deltas (Alg3) -----
        ...
        # committee-to-committee broadcast estimate (use UNIQUE committee members)
        payload_bytes = estimate_chain_record_bytes(payload)
        k_comm = len(set(committee.values()))
        ...
        wb.log({
            ...
            f"cluster/{k}/topk_frac": float(topk_frac),
        })
    ...
    wb.log({
        ...
        "sched/topk_frac": float(topk_frac),
        "sched/stage2_size": int(stage2_size),
    })
...
# Save metrics CSV
# ============================================================
out_csv = os.environ.get("OUT_CSV", "litechain_metrics_comm_latency.csv")
