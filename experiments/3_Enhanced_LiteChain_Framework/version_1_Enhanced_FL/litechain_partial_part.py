# ============================================================
# LiteChain Full: Algorithms 1-6 integrated (FL + LiteChain + DP + W&B SAFE)
# + Partial Participation (rep-weighted + epsilon-greedy)
# + Per-round tuned-threshold evaluation (F1 tuned on VAL) to match baseline/enhanced
#
# ============================================================
...
GROUP   = os.environ.get("WANDB_GROUP", "litechain_full_pp")
RUN_NAME = os.environ.get("WANDB_RUN_NAME", f"litechain_full_pp_{int(time.time())}")
...
# Communication compression simulation (8-bit)
USE_QUANT_COMM = os.environ.get("USE_QUANT_COMM", "1") == "1"

# Global threshold used for quick logging; tuned threshold is computed per-round for comparison
...
# -----------------------------
# Partial participation (NEW)
# -----------------------------
PARTICIPATION_RATE = float(os.environ.get("PARTICIPATION_RATE", "0.5"))  # 0..1
EPS_EXPLORE = float(os.environ.get("EPS_EXPLORE", "0.1"))                # 0..1
MIN_PARTICIPANTS_PER_CLUSTER = int(os.environ.get("MIN_PARTICIPANTS_PER_CLUSTER", "1"))

# -----------------------------
# Per-round threshold tuning (for comparable F1(tuned) logs)
# -----------------------------
...
def estimate_comm_bytes_for_quant_delta(q: Dict[str, np.ndarray], scales: Dict[str, float]) -> int:
    ...
    for _, qi in q.items():
        total += qi.size * 1
        total += 8  # scale float64-ish proxy
    return total
...
# ============================================================
# Signatures (for Algorithm 3 verification step)
# ============================================================
...
# ============================================================
# Algorithm 4: CBFT consensus (explicit verify+commit phases)
# ============================================================
...
def neighbor_clusters_for_device(
    i: int,
    clusters: Dict[int, List[int]],
    k_from: int,
    est_delta_bytes: int,
...
# ============================================================
# Partial Participation Sampler (NEW)
# ============================================================
def sample_participants(
    ...
    if len(client_ids) <= MIN_PARTICIPANTS_PER_CLUSTER:
        return list(client_ids)

    rate = float(max(0.0, min(1.0, PARTICIPATION_RATE)))
    m = int(math.ceil(rate * len(client_ids)))
    m = max(MIN_PARTICIPANTS_PER_CLUSTER, min(m, len(client_ids)))

    rng = np.random.default_rng(RANDOM_STATE + 100000 * (round_id + 1) + 97 * (cluster_id + 1))

    # epsilon-greedy: explore uniformly sometimes
    if rng.random() < EPS_EXPLORE:
        picked = rng.choice(np.array(client_ids), size=m, replace=False)
        return picked.tolist()

    # exploit: reputation-weighted sampling
    w = np.array([max(1e-6, float(reputations.get(cid, 1.0))) for cid in client_ids], dtype=float)
    w = w / (w.sum() + 1e-12)
    picked = rng.choice(np.array(client_ids), size=m, replace=False, p=w)
    return picked.tolist()
...
# Estimate delta bytes for optimization latency proxy
tmpm = new_model()
...
# Training (Rounds)
# ============================================================
rows = []  # for litechain_metrics_full.csv

for r in range(NUM_ROUNDS):
    round_start = time.time()

    # ---------- Algorithm 1/2 optimization ----------
    ...
    # ============================================================
    # Algorithm 3: Intra-cluster training (PARTIAL participation)
    # ============================================================
    for k, all_client_ids in clusters.items():
        ...
        client_ids = sample_participants(all_client_ids, reputations, round_id=r, cluster_id=k)
        ...
        # If somehow no one participated, skip cluster this round
        ...
        for i_idx, cid in enumerate(client_id_list):
        ...
        if len(accepted_idx) == 0:
            ...
            if best_i is None:
                best_i = 0
            accepted_idx = [best_i]
            accepted_client_ids = [client_id_list[best_i]]
            rep_stats[client_id_list[best_i]].accepted += 1

        accept_rate = len(accepted_idx) / max(1, len(client_id_list))

        for cid in all_client_ids:
            reputations[cid] = reputation_score(rep_stats[cid])

        acc_deltas = [client_updates[i] for i in accepted_idx]
        acc_wts = [client_weights[i] * max(1e-3, reputations[client_id_list[i]]) for i in accepted_idx]
        ...
        wb.log({
            "round": r + 1,
            f"cluster/{k}/participants": len(client_id_list),
            f"cluster/{k}/accepted_clients": len(accepted_client_ids),
            f"cluster/{k}/accept_rate": accept_rate,
            f"cluster/{k}/norm_median": med,
            f"cluster/{k}/norm_cutoff": cutoff,
            f"cluster/{k}/val_pr_auc_default": float(val_m_cluster["pr_auc"] or -1.0),
            f"cluster/{k}/val_acc_default": float(val_m_cluster["acc"]),
            f"cluster/{k}/val_f1_default": float(val_m_cluster["f1"]),
        })
    ...
    # ============================================================
    # Global evaluation (DEFAULT + TUNED threshold like baseline/enhanced)
    # ============================================================
    cluster_sizes = []
    cluster_sds = []
    ...
    # tuned threshold for per-round F1(tuned)
    best_t_round, _best_f1_val_round = tune_threshold_for_f1(y_val, val_probs)
    ...
    # PR-AUC independent of threshold:
    val_pr_auc = float(average_precision_score(y_val, val_probs)) if len(np.unique(y_val)) > 1 else -1.0
    test_pr_auc = float(average_precision_score(y_test, test_probs)) if len(np.unique(y_test)) > 1 else -1.0

    round_time = time.time() - round_start
    comm_total_mb = float((comm_client_to_committee + comm_committee_to_committee + comm_chain_bytes) / (1024**2))

    wb.log({
        "round": r + 1,
        "val/pr_auc": val_pr_auc,
        "val/f1_tuned": float(val_m_tuned["f1"]),
        "val/best_threshold": float(best_t_round),
        ...
        "comm/total_MB": comm_total_mb,

        "time/round_sec": float(round_time),
    ...
    print(
        ...
        f"Blocks={len(ledger.blocks)} K={len(clusters)} Comm={comm_total_mb:.3f}MB E2E={round_time:.3f}s"
    )
    ...
    rows.append({
        ...
        "best_threshold_val": float(best_t_round),
...
# ============================================================
# Save per-round comparison CSV
# ============================================================
out_csv = "litechain_metrics_full.csv"
pd.DataFrame(rows).to_csv(out_csv, index=False)
print(f"Saved {out_csv}")


# ============================================================
# Final: threshold tuning on validation (final report)
# ============================================================
...
final_pr_auc = float(average_precision_score(y_test, final_probs_test)) if len(np.unique(y_test)) > 1 else -1.0
final_roc_auc = float(roc_auc_score(y_test, final_probs_test)) if len(np.unique(y_test)) > 1 else -1.0
...
wb.summary_set("final/test_roc_auc", float(final_roc_auc))
wb.summary_set("final/test_pr_auc", float(final_pr_auc))
